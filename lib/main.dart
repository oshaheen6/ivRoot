...
class MedicationCounterApp extends StatelessWidget {
  const MedicationCounterApp({super.key});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MedicationCounterHome(),
    );
  }
}

class MedicationCounterHome extends StatefulWidget {
  const MedicationCounterHome({super.key});

  @override
  State<MedicationCounterHome> createState() => _MedicationCounterHomeState();
}

class _MedicationCounterHomeState extends State<MedicationCounterHome> {
  DateTime selectedDate = DateTime.now();
  Map<String, Map<String, int>> medicationCounts = {};
  // Map structure: {'2024-03-21': {'paracetamol': 2, 'ibuprofen': 1}}

  Map<String, Medication> medications = {
    'Hair': Medication(name: 'Hair', price: 730),
    'Antioxidant': Medication(name: 'Antioxidant', price: 730),
    'Beauty': Medication(name: 'Beauty', price: 730),
    'Elite': Medication(name: 'Elite', price: 730),
    'Energy': Medication(name: 'Energy', price: 730),
  };
  ...
  void incrementMedication(String medication) {
    setState(() {
      String dateKey = DateFormat('yyyy-MM-dd').format(selectedDate);
      medicationCounts[dateKey] ??= {};
      medicationCounts[dateKey]![medication] =
          (medicationCounts[dateKey]?[medication] ?? 0) + 1;
      saveData();
    });
  }
  ...
  Map<String, Map<String, int>> getMonthlyData() {
    String monthYear = DateFormat('yyyy-MM').format(selectedDate);
    return Map.from(medicationCounts)
      ..removeWhere((key, value) => !key.startsWith(monthYear));
  }
